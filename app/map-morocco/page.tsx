...
export default function MoroccoMap({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId
  const { config, isLoading } = useCountryProjection("Morocco", geoUrl)

  if (isLoading) {
    return (
      <CountryMapLayout countryName="Morocco" userId={userId}>
    ...
    <CountryMapLayout countryName="Morocco" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Morocco")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
