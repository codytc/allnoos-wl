...
export default function ZimbabweMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Zimbabwe", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Zimbabwe" userId={userId}>
      ...
      <ComposableMap projection="geoMercator" projectionConfig={config} style={{ width: "100%", height: "100%" }}>
        <ZoomableGroup center={config.center} zoom={1}>
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies
                .filter((geo) => geo.properties.name === "Zimbabwe")
                .map((geo) => (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
