...
export default function SaoTomeAndPrincipeMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("São Tomé and Principe", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="São Tomé and Príncipe" userId={userId}>
    ...
    <CountryMapLayout countryName="São Tomé and Príncipe" userId={userId}>
      <ComposableMap projectionConfig={config} style={{ width: "100%", height: "100%" }}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "São Tomé and Principe")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
                  ...
                  strokeWidth={0.5}
                  style={{
                    default: { fill: "#D4D4D4", outline: "none" },
