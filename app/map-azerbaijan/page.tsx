...
export default function AzerbaijanPage({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId || ""
  const { config, isLoading } = useCountryProjection("Azerbaijan", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Azerbaijan" userId={userId}>
    ...
    <CountryMapLayout countryName="Azerbaijan" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Azerbaijan")
              .map((geo) => (
