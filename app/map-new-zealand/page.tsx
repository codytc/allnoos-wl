...
export default function NewZealandMapPage({
  ...
  const { config, isLoading } = useCountryProjection("New Zealand", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="New Zealand" userId={userId}>
      ...
      <ComposableMap projection="geoMercator" projectionConfig={config} style={{ width: "100%", height: "100%" }}>
        <ZoomableGroup center={config.center} zoom={1}>
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies
                .filter((geo) => geo.properties.name === "New Zealand")
                .map((geo) => (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
