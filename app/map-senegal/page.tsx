...
export default function SenegalMap({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId
  const { config, isLoading } = useCountryProjection("Senegal", geoUrl)

  if (isLoading) {
    return (
      <CountryMapLayout countryName="Senegal" userId={userId}>
    ...
    <CountryMapLayout countryName="Senegal" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Senegal")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
