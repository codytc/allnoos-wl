...
import { Search, Video, Send, Flame, User, Trash2, PinOff, MessageSquareIcon } from "lucide-react"
...
export default function FeedPage() {
  ...
  const currentUserId = 1 // This represents the logged-in user

  const stories: FeedStory[] = allPosts.map(transformPostToStory)
  ...
  const isOwner = allPosts[currentStory]?.userId === currentUserId
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-4 bottom-28 z-30 flex flex-col items-center justify-end gap-8 opacity-90">
        {/* Like button */}
        ...
        >
          <div className="w-14 h-14 flex items-center justify-center">
            <svg
              className={`w-8 h-8 group-active:scale-110 transition-all duration-300 ${
        ...
        >
          <div className="w-14 h-14 flex items-center justify-center">
            <MessageSquareIcon
              className={`group-active:scale-110 transition-all duration-300 size-[30px] ${
                commented[currentStory] ? "text-blue-600" : "text-white"
              }`}
              strokeWidth={2}
            />
          </div>
          <span className="text-sm font-semibold text-white">{story.comments}</span>
        </Link>

        <Link
          href="/camera"
          className="flex flex-col items-center gap-0.5 group relative pointer-events-auto"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="w-14 h-14 flex items-center justify-center">
            <Video className="group-active:scale-110 transition-transform duration-200 size-11 ml-2 text-white" />
          </div>
        </Link>
      </div>
      ...
      <div className="absolute bottom-24 left-0 right-0 z-20 px-4">
        <div className="mb-4 mr-20">
          ...
          <Link href={`/user-profile?userId=${allPosts[currentStory]?.userId}`} className="flex items-center gap-3">
            ...
            <div className="flex-1 flex flex-col justify-center">
              <span className="text-white font-semibold opacity-80 mb-1">{story.author.name}</span>
