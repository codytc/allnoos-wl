...
export default function TogoMap({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId
  const { config, isLoading } = useCountryProjection("Togo", geoUrl)

  if (isLoading) {
    return (
      <CountryMapLayout countryName="Togo" userId={userId}>
    ...
    <CountryMapLayout countryName="Togo" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Togo")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
