...
export default function ItalyMapPage({
  ...
  const { config, isLoading } = useCountryProjection("Italy", geoUrl)
  ...
  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Italy" userId={userId}>
    ...
    <CountryMapLayout countryName="Italy" userId={userId}>
      ...
      >
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies.map((geo) => {
              const regionName = geo.properties.NOME_REG || geo.properties.name || "Unknown"
