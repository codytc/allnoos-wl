...
export default function VanuatuMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Vanuatu", geoUrl)

  return (
    <CountryMapLayout countryName="Vanuatu" userId={userId}>
      {isLoading ? (
        ...
        <ComposableMap projection="geoMercator" projectionConfig={config}>
          <ZoomableGroup zoom={1}>
            <Geographies geography={geoUrl}>
              {({ geographies }) =>
                geographies
                  .filter((geo) => geo.properties.name === "Vanuatu")
                  .map((geo) => (
