...
export default function GeorgiaPage({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId || ""
  const { config, isLoading } = useCountryProjection("Georgia", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Georgia" userId={userId}>
    ...
    <CountryMapLayout countryName="Georgia" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Georgia")
              .map((geo) => (
