...
export default function MadagascarMapPage({
  ...
  const { config, isLoading } = useCountryProjection("Madagascar", geoUrl)
  const userId = searchParams.userId ? Number.parseInt(searchParams.userId) : 1

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Madagascar" userId={userId}>
      ...
      >
        <ZoomableGroup center={[config.center[0], config.center[1]]} zoom={1}>
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies
                .filter((geo) => geo.properties.name === "Madagascar")
                .map((geo) => (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
