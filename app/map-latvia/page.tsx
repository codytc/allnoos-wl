...
export default function LatviaMap({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId
  const { config, isLoading } = useCountryProjection("Latvia", geoUrl)

  if (isLoading) {
    return (
      <CountryMapLayout countryName="Latvia" userId={userId}>
    ...
    <CountryMapLayout countryName="Latvia" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Latvia")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
