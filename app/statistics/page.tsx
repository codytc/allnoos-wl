...
import { ArrowLeft, Users, Clock, Flame, MessageSquareIcon, X, TrendingUp, Target, Lightbulb } from "lucide-react"
  ...
  const renderAveragingBar = (currentValue: string, averageValue: string) => {
    ...
    return (
      <div className="flex rounded-full overflow-hidden items-center border-2 border-gray-500 size-auto h-5 mb-0">
        ...
        >
          <span className={`text-xs font-semibold ${status === "below" ? "text-white" : "text-gray-700"}`}>BELOW</span>
        </div>
        <div className={`bg-gray-300 h-8 flex-1 flex items-center justify-center relative`}>
          <span className={`text-xs font-semibold text-gray-600`}>AVERAGE {averageValue}</span>
        </div>
        <div
          className={`bg-yellow-400 h-8 flex-1 flex items-center justify-center relative ${status === "above" ? "" : "opacity-30"}`}
        >
