...
export default function PalauMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Palau", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Palau" userId={userId}>
    ...
    <CountryMapLayout countryName="Palau" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config} className="w-full h-full">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Palau")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
