...
export default function TanzaniaMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const { config, isLoading } = useCountryProjection("Tanzania", geoUrl)
  const userId = searchParams.userId ? Number.parseInt(searchParams.userId) : 1

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Tanzania" userId={userId}>
      ...
      <ComposableMap projection="geoMercator" projectionConfig={config} style={{ width: "100%", height: "100%" }}>
        <ZoomableGroup center={[config.center[0], config.center[1]]} zoom={1}>
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies
                .filter((geo) => geo.properties.name === "Tanzania")
                .map((geo) => (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
