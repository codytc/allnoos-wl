...
export default function TurkeyMapPage({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Turkey", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Turkey" userId={userId}>
      ...
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <ZoomableGroup center={config.center as [number, number]} zoom={1}>
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies
                .filter((geo: any) => geo.properties.iso_a2 === "TR")
                .map((geo: any) => (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
