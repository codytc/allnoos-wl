...
export default function BeninMap({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId
  const { config, isLoading } = useCountryProjection("Benin", geoUrl)

  if (isLoading) {
    return (
      <CountryMapLayout countryName="Benin" userId={userId}>
    ...
    <CountryMapLayout countryName="Benin" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Benin")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
