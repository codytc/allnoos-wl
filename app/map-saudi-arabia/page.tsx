...
export default function SaudiArabiaMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const { config, isLoading } = useCountryProjection("Saudi Arabia", geoUrl, "SA")
  const userId = searchParams.userId ? Number.parseInt(searchParams.userId) : 1

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Saudi Arabia" userId={userId}>
      ...
      >
        <ZoomableGroup center={[config.center[0], config.center[1]]} zoom={1}>
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies
                .filter((geo: any) => geo.properties.iso_a2 === "SA")
                .map((geo) => (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
