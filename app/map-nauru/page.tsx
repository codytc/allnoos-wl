...
export default function NauruMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Nauru", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Nauru" userId={userId}>
    ...
    <CountryMapLayout countryName="Nauru" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config} className="w-full h-full">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Nauru")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
