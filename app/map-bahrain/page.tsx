...
export default function BahrainPage({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId || ""
  const { config, isLoading } = useCountryProjection("Bahrain", geoUrl)

  return (
    <CountryMapLayout countryName="Bahrain" userId={userId}>
      {isLoading ? (
        ...
        <ComposableMap projection="geoMercator" projectionConfig={config}>
          <ZoomableGroup zoom={1}>
            <Geographies geography={geoUrl}>
              {({ geographies }) =>
                geographies
                  .filter((geo) => geo.properties.name === "Bahrain")
                  .map((geo) => (
