...
export default function ElSalvadorMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("El Salvador", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="El Salvador" userId={userId}>
    ...
    <CountryMapLayout countryName="El Salvador" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config} className="w-full h-full">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "El Salvador")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
