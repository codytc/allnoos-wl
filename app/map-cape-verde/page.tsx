...
export default function CapeVerdeMapPage({
  ...
  const { config, isLoading } = useCountryProjection("Cape Verde", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Cape Verde" userId={userId}>
    ...
    <CountryMapLayout countryName="Cape Verde" userId={userId}>
      <ComposableMap projectionConfig={config} style={{ width: "100%", height: "100%" }}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Cape Verde")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
