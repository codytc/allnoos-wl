...
export default function CostaRicaMapPage({
  ...
  const { config, isLoading } = useCountryProjection("Costa Rica", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Costa Rica" userId={userId}>
    ...
    <CountryMapLayout countryName="Costa Rica" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config} className="w-full h-full">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Costa Rica")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
