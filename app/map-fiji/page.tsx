...
export default function FijiMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Fiji", geoUrl)

  return (
    <CountryMapLayout countryName="Fiji" userId={userId}>
      {isLoading ? (
        ...
        <ComposableMap projection="geoMercator" projectionConfig={config}>
          <ZoomableGroup zoom={1}>
            <Geographies geography={geoUrl}>
              {({ geographies }) =>
                geographies
                  .filter((geo) => geo.properties.name === "Fiji")
                  .map((geo) => (
