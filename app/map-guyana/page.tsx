...
export default function GuyanaMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Guyana", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Guyana" userId={userId}>
    ...
    <CountryMapLayout countryName="Guyana" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config} className="w-full h-full">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Guyana")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
