...
export default function EstoniaMap({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId
  const { config, isLoading } = useCountryProjection("Estonia", geoUrl)

  if (isLoading) {
    return (
      <CountryMapLayout countryName="Estonia" userId={userId}>
    ...
    <CountryMapLayout countryName="Estonia" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Estonia")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
