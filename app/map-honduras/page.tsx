...
export default function HondurasMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Honduras", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Honduras" userId={userId}>
    ...
    <CountryMapLayout countryName="Honduras" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config} className="w-full h-full">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Honduras")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
