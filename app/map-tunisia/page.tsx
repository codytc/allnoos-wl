...
export default function TunisiaMap({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId
  const { config, isLoading } = useCountryProjection("Tunisia", geoUrl)

  if (isLoading) {
    return (
      <CountryMapLayout countryName="Tunisia" userId={userId}>
    ...
    <CountryMapLayout countryName="Tunisia" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Tunisia")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
