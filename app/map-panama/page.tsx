...
export default function PanamaMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Panama", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Panama" userId={userId}>
    ...
    <CountryMapLayout countryName="Panama" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config} className="w-full h-full">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Panama")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
