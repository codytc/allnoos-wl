...
export default function LesothoMap({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId
  const { config, isLoading } = useCountryProjection("Lesotho", geoUrl)

  if (isLoading) {
    return (
      <CountryMapLayout countryName="Lesotho" userId={userId}>
    ...
    <CountryMapLayout countryName="Lesotho" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Lesotho")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
