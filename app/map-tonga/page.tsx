...
export default function TongaMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Tonga", geoUrl)

  return (
    <CountryMapLayout countryName="Tonga" userId={userId}>
      {isLoading ? (
        ...
        <ComposableMap projection="geoMercator" projectionConfig={config}>
          <ZoomableGroup zoom={1}>
            <Geographies geography={geoUrl}>
              {({ geographies }) =>
                geographies
                  .filter((geo) => geo.properties.name === "Tonga")
                  .map((geo) => (
