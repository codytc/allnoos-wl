...
export default function NicaraguaMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Nicaragua", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Nicaragua" userId={userId}>
    ...
    <CountryMapLayout countryName="Nicaragua" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config} className="w-full h-full">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Nicaragua")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
