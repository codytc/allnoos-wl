...
export default function KuwaitPage({ searchParams }: { searchParams: { userId?: string } }) {
  const userId = searchParams.userId || ""
  const { config, isLoading } = useCountryProjection("Kuwait", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Kuwait" userId={userId}>
    ...
    <CountryMapLayout countryName="Kuwait" userId={userId}>
      <ComposableMap projection="geoMercator" projectionConfig={config}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Kuwait")
              .map((geo) => (
