...
export default function ComorosMapPage({
  searchParams,
}: {
  searchParams: { userId?: string }
}) {
  const userId = searchParams.userId || "1"
  const { config, isLoading } = useCountryProjection("Comoros", geoUrl)

  if (isLoading || !config) {
    return (
      <CountryMapLayout countryName="Comoros" userId={userId}>
    ...
    <CountryMapLayout countryName="Comoros" userId={userId}>
      <ComposableMap projectionConfig={config} style={{ width: "100%", height: "100%" }}>
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies
              .filter((geo) => geo.properties.name === "Comoros")
              .map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
